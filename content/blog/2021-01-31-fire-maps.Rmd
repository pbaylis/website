...
The next step is to count the number of overlapping fire areas in the county. Here we'll use a neat (an not very well-documented) trick: `st_intersection`, an `sf` function, can figure out where polygons overlap and how many of them overlap in each place. This is a little time-consuming, but once it's done we can arrange the overlaps by their number (from least to most) and plot with `ggplot2`.
