...
description: "Generating predictions and standard errors for response functions with fixed effects models"
...
::: {.callout-note}
## 2025 Update
The approach detailed here has been made (mostly) obsolete by the `comparisons()` function in the [marginaleffects](https://github.com/vincentarelbundock/marginaleffects) package. I wrote up a similar post using that package [here](/posts/2025-09-25-predict-update/). That being said, the `predict_partial` function below is still useful to me when I'm trying to do something very particular, so I leave it up for posterity.
:::
...
![Carleton and Hsiang (2016), Figure 3](carleton-hsiang-fig3.png)
...
predict_partial <- function(object, newdata, se.fit = FALSE,
    ...
    # object <- fit_lm; newdata <- newdata; se.fit = T; interval = "confidence"; level = 0.95
    ...
    if (class(object) == "fixest") {
        attr(Terms, "intercept") <- 0 # Remove intercept for fixest objects
        B <- as.numeric(coef(object))
        df <- attributes(vcov(fit_feols, attr = T))$df.K
...
Note that this function also works with `lm`, though I'm not sure why you would use it given that `predict.lm` provides all of the functionality above and more.
