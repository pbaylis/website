---
title: "Nicer LaTeX tables in R"
description: "A 2024 update on how to streamline making better-looking LaTeX tables in R"
...
First, we gotta load some stuff. I'm going to use [`pacman`](https://trinker.github.io/pacman/vignettes/Introduction_to_pacman.html) for package management, the [`tidyverse`](https://www.tidyverse.org/) for general data manipulation, `AER` for some data on automobile fatalities, [`modelsummary`](https://modelsummary.com/) for summary statistics, [`kableExtra`](https://bookdown.org/yihui/rmarkdown-cookbook/kableextra.html) for some extra table formatting, and the inimitable [`fixest`](https://lrberge.github.io/fixest/) for estimating regressions and making the regression tables. 
...
Now we can combine the two tables and save them as `.tex` files. This requires a tiny bit of hacking to jam the tables together. Note the call to `kbl` and the options it uses yield only the `tabular` portion of the table in the output. This is useful because it lets me set captions and table notes in the LaTeX document itself.
...
My tables often use similar sets of control variables with different sets of fixed effects. Content note: this is a regression of annual automobile fatalities on a few variables, including the unemployment rate, income, beer taxes, and miles driven. The causal inference economist in me is required to disclose that I don't think we're estimating true causal effects here, but it's a nice example for a table. I use `etable` at the end of this chunk for a quick view of the estimates.
